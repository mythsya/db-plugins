...
namespace QuestTools
{
    ...
    public static class CustomConditions
    {
        ...
        {
            var nearbyActors = ZetaDia.Actors.GetActorsOfType<DiaObject>(true).Where(i => i.IsValid && i.ActorSNO == actorId && Vector3.Distance(i.Position, ZetaDia.Me.Position) <= range).ToList();
        ...
        {
            var result = ZetaDia.ActInfo.Bounties.FirstOrDefault(q => q.Quest == (SNOQuest)questId && q.LevelAreas.Contains((SNOLevelArea)ZetaDia.CurrentLevelAreaId) || q.StartingLevelArea == (SNOLevelArea)ZetaDia.CurrentLevelAreaId);
