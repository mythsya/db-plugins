...
namespace QuestTools.ProfileTags
{
    ...
    public class ExploreDungeonTag : ProfileBehavior, IEnhancedProfileBehavior
    {
        ...
        private DiaObject CurrentActor
        {
            get
            {
                var actor = ZetaDia.Actors.GetActorsOfType<DiaObject>(true)
                    .Where(diaObj => 
                    diaObj.IsValid && 
                    (diaObj.ActorSNO == ActorId || Objectives.Any(o => o.ActorId != 0 && o.ActorId == diaObj.ActorSNO)) &&
        ...
        private Composite InvestigateActor()
        {
            return new ActionRunCoroutine(async ret =>
            {
                ...
                if (actor != null && actor.IsValid && actor.Position.Distance2DSqr(MyPosition) >= ObjectDistance * ObjectDistance)
                   //await CommonCoroutines.MoveTo(actor.Position, string.Format("InvestigateActor {0} {1} {2}", actor.ActorSNO, actor.Name, actor.ActorType));
                    await Navigator.MoveTo(actor.Position, string.Format("InvestigateActor {0} {1} {2}", actor.ActorSNO, actor.Name, actor.ActorType));
            });
        }
        ...
        private Composite CheckIsObjectiveFinished()
        {
            ...
            new PrioritySelector(
                ...
                new Decorator(ret => (EndType == ExploreEndType.ObjectFound || EndType == ExploreEndType.SceneLeftOrActorFound || EndType == ExploreEndType.ActorFoundOrBountyComplete) && ActorId != 0 && ZetaDia.Actors.GetActorsOfType<DiaObject>(true)
                    .Any(a => a.ActorSNO == ActorId && !GetIsGuardedBountyGizmoActivated(a) && a.Distance <= ObjectDistance),
                    new Sequence(
        ...
        private static bool GetIsGuardedBountyGizmoActivated(DiaObject actor)
        {
            ...
            if (!DataDictionary.GuardedBountyGizmoIds.Contains(actor.ActorSNO))
            {
                return false;
            }
        ...
        {
            return AlternateActors.Any() && ZetaDia.Actors.GetActorsOfType<DiaObject>(true).Any(o => AlternateActors.Any(a => a.ActorId == o.ActorSNO && o.Distance <= a.ObjectDistance) && !GetIsGuardedBountyGizmoActivated(o));
        }
        ...
        private void FindPrioritySceneTarget()
        {
            ...
            var scenes = ZetaDia.Scenes.Where(s => s.SceneGuid == -1);
            ...
            foreach (Scene scene in priorityScenes)
            {
                ...
                {
                    Logger.Error("Scene {0} has a null NavZone! SceneGUID={1}", scene.Name, scene.SceneGuid);
                }
        ...
        public bool GetIsBountyDone()
        {
            try
            {
                ...
                var b = ZetaDia.ActInfo.Bounties.FirstOrDefault(q => q.State == QuestState.InProgress && (q.LevelAreas.Contains((SNOLevelArea)ZetaDia.CurrentLevelAreaId) || q.StartingLevelArea == (SNOLevelArea)ZetaDia.CurrentLevelAreaId));
