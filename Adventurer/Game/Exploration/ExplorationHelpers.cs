        ...
        public static ExplorationNode NearestWeightedUnvisitedNode(HashSet<int> levelAreaIds, List<string> ignoreScenes = null)
        {
            ...
            using (new PerformanceLogger("NearestUnvisitedNodeLocation", true))
            {
                ...
                if (node == null)
                {
                    ...
                    //    n =>
                    //        !n.IsVisited && n.DynamicWorldId == dynamicWorldId &&
                    //        n.NavigableCenter.DistanceSqr(myPosition) > 100 &&
                    //        levelAreaIds.Contains(n.LevelAreaId) && n.GetNeighbors(3).Count(nn => n.HasEnoughNavigableCells) > 1)
                ...
                if (node == null)
                {
                    ...
                    Logger.Info("[ExplorationLogic] Couldn't find any unvisited nodes. Current AdvDia.LevelAreaId: {0}, ZetaDia.CurrentLevelAreaId: {3}, Total Nodes: {1} Unvisited Nodes: {2} Searching In [{4}]", AdvDia.CurrentLevelAreaId, allNodes, unvisitedNodes, ZetaDia.CurrentLevelAreaId, string.Join(", ", levelAreaIds));
                }
                return node;
