...
namespace Trinity.Items
{
    public class TrinityItemManager : ItemManager
    {
        ...
        public override bool ShouldSellItem(ACDItem item)
        {
            ...
            {
                Logger.LogVerbose("Selling item {0} ({1}) because it can't be salvaged (vendor bought)", item.Name, item.ActorSNO);
                action = true;
            }
        ...
        public bool ShouldStashItem(ACDItem item, ItemEvaluationType evaluationType)
        {
            ...
            if (Trinity.Settings.Loot.ItemFilterMode == ItemFilterMode.TrinityWithItemRules && Trinity.Settings.Loot.ItemRules.AlwaysStashAncients && item.AncientRank > 0)
            {
                Logger.Log(TrinityLogLevel.Info, LogCategory.UserInformation, "{0} [{1}] = (STASHING: ItemRules Option - Always Stash Ancients)", cItem.RealName, cItem.InternalName);
                return true;
            }

            // ItemList - Always stash ancients setting
            if (Trinity.Settings.Loot.ItemFilterMode == ItemFilterMode.ItemList && Trinity.Settings.Loot.ItemList.AlwaysStashAncients && item.AncientRank > 0)
            {
                Logger.Log(TrinityLogLevel.Info, LogCategory.UserInformation, "{0} [{1}] = (STASHING: ItemList Option - Always Stash Ancients)", cItem.RealName, cItem.InternalName);
                return true;
            }
            ...
            if (Trinity.Settings.Loot.ItemFilterMode == ItemFilterMode.ItemList && Trinity.Settings.Loot.ItemList.AlwaysTrashNonAncients && 
                item.AncientRank == 0 && item.ItemQualityLevel >= ItemQuality.Legendary && isArmorWeaponOrJewellery)
            {
                Logger.Log(TrinityLogLevel.Info, LogCategory.UserInformation, "{0} [{1}] = (TRASHING: ItemList Option - Always Sell/Salvage Non-Ancients)", cItem.RealName, cItem.InternalName);
                return false;
            }
            ...
            if (tItemType == TrinityItemType.CraftingMaterial)
            {
                ...
                if (evaluationType == ItemEvaluationType.Keep && craftMaterialType != InventoryItemType.None)
                {
...
#pragma warning disable 1718
        ...
        internal static Vector2 FindValidBackpackLocation(bool isOriginalTwoSlot, bool forceRefresh = false)
        {
            using (new PerformanceLogger("FindValidBackpackLocation"))
            {
                try
                {
                    if (!forceRefresh && _lastBackPackLocation != new Vector2(-2, -2) &&
