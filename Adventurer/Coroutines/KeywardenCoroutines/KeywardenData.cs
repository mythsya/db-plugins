...
namespace Adventurer.Coroutines.KeywardenCoroutines
{
    public class KeywardenData
    {
        ...
        public long MachinesCount
        {
            get
            {
                var stashCount = ZetaDia.Me.Inventory.StashItems.Where(i => i.IsValid && i.ActorSNO == KeySNO).Sum(i=>i.ItemStackQuantity);
                var backpackCount= ZetaDia.Me.Inventory.Backpack.Where(i => i.IsValid && i.ActorSNO == KeySNO).Sum(i => i.ItemStackQuantity);
