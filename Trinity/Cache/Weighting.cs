    ...
    public partial class Trinity
    {
        ...
        private static void RefreshDiaGetWeights()
        {
            ...
            using (new PerformanceLogger("RefreshDiaObjectCache.Weighting"))
            {
                ...
                bool IsHighLevelGrift = Player.TieredLootRunlevel > 45;
                ...
                var isBlockedByMonsters = ObjectCache.Count(u => u.HitPoints > 0 && u.Distance <= 8f) >= 10;
                ...
                if (Settings.Combat.Misc.GoblinPriority == GoblinPriority.Kamikaze)
                {
                    ...
                    if (goblin != null && !isStuck && !isBlockedByMonsters)
                    {
                        ...
                        Logger.Log("Going Kamakazi on Goblin '{0} ({1})' Distance={2}", goblin.InternalName, goblin.ActorSNO, goblin.Distance);
                ...
                if (monstersWithReflectActive.Any() && Settings.Combat.Misc.IgnoreMonstersWhileReflectingDamage)
                    Logger.Log(LogCategory.Targetting, "One or more monsters currently have reflect buff active!");
                ...
                foreach (TrinityCacheObject cacheObject in ObjectCache.OrderBy(c => c.Distance))
                {
                    ...
                    if (CombatBase.IsDoingGoblinKamakazi && cacheObject.IsTreasureGoblin && cacheObject.RActorGuid == CombatBase.KamakaziGoblin.RActorGuid)
                    {
                        cacheObject.Weight = MaxWeight;
                        continue;                           
                    }

                    if (CombatBase.IsDoingGoblinKamakazi && cacheObject.Type != TrinityObjectType.Door && !isStuck && !isBlockedByMonsters)
                        continue;                                           
                        ...
                        case TrinityObjectType.Unit:
                            {
                                ...
                                {
                                    CombatBase.IsDoingGoblinKamakazi = true;
                                    Logger.Log("Goblin Kamakazi!");
                                    objWeightInfo += "GoblinKamikaze ";
                                    cacheObject.Weight = MaxWeight;
                                    break;
                                }

                                if (PlayerMover.IsBlocked && cacheObject.Distance >= 12f && !CombatBase.IsDoingGoblinKamakazi && !cacheObject.IsBoss)
                                {
                                    objWeightInfo += "IsBlocked ";
                                ...
                                if (cacheObject.IsTrashMob)
                                {
                                    ...
                                    // Except if has been primary target or if already low on health
                                    if ((shouldIgnoreTrashMob && !isInHotSpot && !(Player.ActorClass == ActorClass.DemonHunter && IsHighLevelGrift) && 
                                        !cacheObject.IsQuestMonster && !cacheObject.IsMinimapActive && !ignoreSummoner &&
                                        !cacheObject.IsBountyObjective) || Player.CurrentHealthPct <= 0.6 || getHiPriorityContainer || getHiPriorityShrine || goblinKamikaze)
                                    {
                                ...
                                {
                                    if (Settings.Combat.Misc.IgnoreMonstersWhileReflectingDamage && monstersWithReflectActive.Any(u => u.RActorGuid == cacheObject.RActorGuid))
                                    {
                                        Logger.Log(LogCategory.Weight, "Ignoring {0} due to reflect damage buff", cacheObject.InternalName);
                                ...
                                // If a reflecting monster is near to this, reduce weight
                                if(monstersWithReflectActive.Any(u => u.Position.Distance(cacheObject.Position) < 15f))
                                {
                                    objWeightInfo += "NearReflectDamage ";
                                    cacheObject.Weight -= 10000;
                                }
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        // Boost close range clusters
                                        if (cacheObject.Distance <= 8f && cacheObject.NearbyUnitsWithinDistance(12f) > 5)
                        ...
                        case TrinityObjectType.ProgressionGlobe:
                            {
                                ...
                                if (cacheObject.GoldAmount <= 0 && cacheObject.Distance <= 40f)
                                    cacheObject.Weight += 5000d;
                        ...
                        case TrinityObjectType.Shrine:
                            {
                                ...
                                // Get close when elites are present 
                                if (getHiPriorityShrine && elites.Any() && cacheObject.Distance < 30f)
                                {
                                    cacheObject.Weight = MaxWeight;
                                    break;
                                }
                                    
                                // Always get shrines near a solo boss
                                if (elites.Count == 1 && elites.First().IsBoss)
                                {
                                    cacheObject.Weight = MaxWeight;
                                    break;
                                }
                                ...
                                if (Settings.WorldObject.HiPriorityShrines)
                                {
                                    ...
                                    cacheObject.Weight = Math.Max(MaxWeight*Math.Pow((0.99), cacheObject.RadiusDistance), 100d);
                        ...
                        case TrinityObjectType.Interactable:
                            {
                                ...
                                if (cacheObject.Object is DiaGizmo && cacheObject.Gizmo != null && cacheObject.Gizmo.CommonData != null && cacheObject.Gizmo.CommonData.ActorInfo != null && cacheObject.Gizmo.CommonData.ActorInfo.GizmoType == GizmoType.Headstone &&
                        ...
                        case TrinityObjectType.Container:
                            {
                                ...
                                var isRare = cacheObject.InternalName.ToLower().Contains("chest_rare") || DataDictionary.ResplendentChestIds.Contains(cacheObject.ActorSNO);
